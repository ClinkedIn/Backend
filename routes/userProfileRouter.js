...
router.route('/me').get(protect, user.getMe);

router.route('/profile').patch(protect, user.editIntro);

router.route('/contact-info').patch(protect, user.editContactInfo);

router.route('/companies').get(protect, user.getCompanies);

router.route('/about').patch(protect, user.editAbout);

router.route('/privacy-settings').patch(protect, user.updatePrivacySettings);
...
router
    .route('/skills/endorsements/add-endorsement')
    .post(protect, user.addEndorsement);

router
    .route('/skills/endorsements/remove-endorsement/:skillName')
...
// 5. MEDIA ROUTES
router
    .route('/pictures/profile-picture')
...
router.route('/search').get(protect, user.searchUsers); // Search for users by name, company, or industry

router.route('/search/users').get(protect, user.searchUsersByName); // Specific endpoint for name search
...
router
    .route('/connections/requests/:senderId')
    .patch(protect, user.handleConnectionRequest); // Accept/Decline connection request

router.route('/connections').get(protect, user.getConnectionsList); // Get list of connections
...
router.route('/blocked').get(protect, user.getBlockedUsers);
...
router.route('/message-requests').get(protect, user.getMessageRequests); // Get list of message requests

router
    .route('/message-requests/:requestId')
...
router.route('/:userId/user-activity').get(protect, user.getUserActivity);

router.route('/:userId').get(protect, user.getUserProfile);
