...
const updateSkillExperienceReferences = (
  user,
  experienceIndex,
  newSkills = [],
  oldSkills = []
  ...
  for (const skillName of oldSkills) {
    if (!newSkills.includes(skillName)) {
      ...
      if (skillIndex !== -1) {
        user.skills[skillIndex].experience = user.skills[
          skillIndex
        ].experience.filter((i) => i !== experienceIndex);

        // Remove skill if it's no longer referenced anywhere
        if (
          user.skills[skillIndex].experience.length === 0 &&
          user.skills[skillIndex].education.length === 0
        ) {
...
const handleUserPicture = async (req, res, fieldName, isDelete = false) => {
  try {
    ...
    console.error(
      `Error ${isDelete ? "deleting" : "uploading"} ${fieldName}:`,
      error
    );
    res.status(500).json({
...
const checkUserAccessPermission = async (
  user,
  requesterId,
  requester = null,
  accessType = "view"
  ...
  try {
    ...
    if (
      ...
      requester.blockedUsers.includes(user._id.toString())
    ) {
      return {
        hasAccess: false,
    ...
    if (user.profilePrivacySettings === "connectionsOnly") {
      ...
      const isConnected =
        requester.connectionList &&
        requester.connectionList.some(
