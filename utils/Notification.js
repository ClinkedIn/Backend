...
const generateMessage = (sendingUser, subject, resource) => {
    if (subject === 'impression') {
        if (resource.targetType === 'Post') {
            return notificationTemplate.reactionPost(
                sendingUser,
                resource.type
            );
        } else if (resource.targetType === 'Comment') {
            return notificationTemplate.reactionComment(
                sendingUser,
                resource.type
            );
        }
    } else if (subject === 'connection request') {
        return notificationTemplate.connectionRequest(sendingUser);
    }
