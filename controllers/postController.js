...
const createPost = async (req, res) => {
  ...
  try {
    ...
    await sendCreationPostNotification(user, newPost);
    res.status(201).json({
...
const unsavePost = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Post removed from saved posts successfully" });
  } catch (error) {
...
const deleteRepost = async (req, res) => {
  try {
    ...
    if (post && post.repostCount > 0) {
      ...
      await postModel.findByIdAndUpdate(postId, { $inc: { repostCount: -1 } });
    }
