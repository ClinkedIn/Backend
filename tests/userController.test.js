...
describe("registerUser", () => {
    ...
    it("should return 400 for invalid email", async () => {
        ...
        await userController.registerUser(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ message: "Email not valid, Write a valid email" });
    });
    ...
    it("should return 409 if user already exists", async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({ message: "The User already exist use another email" });
    });
...
describe("resendConfirmationEmail", () => {
    ...
    it("should return 400 if user is already confirmed", async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ message: "User is already confirmed!!" });
    });
...
describe("googleLogin", () => {
    ...
    it("should update googleId if user exists with null googleId", async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({
            status: "success",
            message: undefined,
            data: { accessToken: "access", refreshToken: "refresh", user: existingUser },
        });
    });
...
describe("forgotPassword", () => {
    ...
    it("should throw error if user not found", async () => {
        ...
        userModel.findOne.mockResolvedValue(null);
        await expect(userController.forgotPassword(req, res)).rejects.toThrow("There is no such email address");
    });
    ...
    it("should return 500 if forgot password email fails", async () => {
        ...
        req.body = { email: "john@example.com" };
        userModel.findOne.mockResolvedValue(user);
        sendForgotPasswordEmail.mockResolvedValue({ success: false, error: "Email error" });
        req.protocol = "http";
        req.get = jest.fn().mockReturnValue("localhost");
...
describe("resetPassword", () => {
    ...
    it("should return 400 if token is invalid or expired", async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ message: "Token is invalid or has expired" });
    });
...
describe("updatePassword", () => {
    ...
    it("should return 401 if current password is wrong", async () => {
        ...
        req.user = { password: "hashed", correctPassword: jest.fn().mockResolvedValue(false) };
        await userController.updatePassword(req, res);
        expect(req.user.correctPassword).toHaveBeenCalledWith("wrong", "hashed");
        expect(res.status).toHaveBeenCalledWith(401);
        expect(res.json).toHaveBeenCalledWith({ message: "your current password is wrong" });
    });

    it("should return 400 if new password is invalid", async () => {
        req.body = { passwordCurrent: "current", password: "invalid" };
        req.user = { password: "hashed", correctPassword: jest.fn().mockResolvedValue(true) };
        validatePassword.mockReturnValue(false);
        await userController.updatePassword(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({
...
describe("updateEmail", () => {
    ...
    it("should return 401 if current password is wrong", async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(401);
        expect(res.json).toHaveBeenCalledWith({ message: "please enter the correct password" });
    });
    ...
    it("should update email successfully", async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({
            status: "success",
            message: "Email updated successfully. Please check your email to confirm your account.",
            data: { accessToken: "access", refreshToken: "refresh", user },
        });
    });
});
...
describe("updateName", () => {
    let req, res;
    beforeEach(() => {
        req = { body: { firstName: "Jane", lastName: "Doe" }, user: { id: "userId" } };
        res = mockRes();
    });
    ...
    it("should update name successfully", async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({
            ...
            data: { accessToken: "access", refreshToken: "refresh", user: updatedUser },
        });
    });
});
...
// const { sendEmailConfirmation, sendForgotPasswordEmail } = require('../utils/emailService');
// const { verifyCaptcha } = require('../utils/verifyCaptcha');
// const { generateTokens } = require('../controllers/jwtController');
// const firebaseAdmin = require('../utils/firebase');
...
// jest.mock('../utils/emailService');
// jest.mock('../utils/verifyCaptcha');
// jest.mock('../controllers/jwtController');
// jest.mock('../utils/firebase');
// jest.mock('crypto');
// jest.mock('axios');
...
//                 recaptchaResponseToken: 'validToken'
//             };
...
//             });
//             sendEmailConfirmation.mockResolvedValue({ success: true });
//         });
...
//             expect(sendEmailConfirmation).toHaveBeenCalledWith('newUserId');
...
//             verifyCaptcha.mockResolvedValue(false);
//             await registerUser(req, res);
//             expect(verifyCaptcha).toHaveBeenCalledWith('validToken');
//             expect(res.status).toHaveBeenCalledWith(400);
//             expect(res.json).toHaveBeenCalledWith({
//                 success: false,
//                 errors: [{ msg: 'reCAPTCHA verification failed. Please try again.' }]
...
//             sendEmailConfirmation.mockRejectedValue(new Error('Email sending failed'));
//             console.log = jest.fn();
//             await registerUser(req, res);
//             expect(userModel.create).toHaveBeenCalled();
//             expect(console.log).toHaveBeenCalledWith('Mail sending failed', expect.any(Error));
//             expect(res.status).toHaveBeenCalledWith(201);
...
//         it('should return 401 if password is incorrect', async () => {
//             mockUser.correctPassword.mockResolvedValue(false);
...
//                 firebase: { identities: { 'google.com': ['googleUid'] } }
//             };
//             firebaseAdmin.auth().verifyIdToken = jest.fn().mockResolvedValue(decodedToken);
//         });

//         it('should login with Google successfully for existing user', async () => {
//             mockUser.googleId = 'googleUid';
//             userModel.findOne.mockResolvedValue(mockUser);
//             await googleLogin(req, res);
//             expect(firebaseAdmin.auth().verifyIdToken).toHaveBeenCalledWith('validFirebaseToken');
//             expect(userModel.findOne).toHaveBeenCalledWith({ email: 'john@example.com' });
//             expect(generateTokens).toHaveBeenCalled();
//             expect(res.status).toHaveBeenCalledWith(200);
//             expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
...
//         it('should create new user when logging with Google for first time', async () => {
//             userModel.findOne.mockResolvedValue(null);
//             userModel.create.mockResolvedValue({
//                 _id: 'newUserId',
//                 firstName: 'John Doe',
...
//         it('should update Google ID for existing user without Google ID', async () => {
//             mockUser.googleId = null;
//             mockUser.save = jest.fn().mockResolvedValue(true);
//             userModel.findOne.mockResolvedValue(mockUser);
//             await googleLogin(req, res);
//             expect(mockUser.save).toHaveBeenCalled();
//             expect(mockUser.googleId).toBe('googleUid');
//             expect(generateTokens).toHaveBeenCalled();
//             expect(res.status).toHaveBeenCalledWith(200);
//         });

//         it('should return 401 if Google ID does not match', async () => {
//             mockUser.googleId = 'differentGoogleId';
//             userModel.findOne.mockResolvedValue(mockUser);
//             await googleLogin(req, res);
//             expect(res.status).toHaveBeenCalledWith(401);
//             expect(res.json).toHaveBeenCalledWith({ message: 'Unauthorized' });
...
//         beforeEach(() => {
//             req.body = { email: 'john@example.com' };
//             mockUser.createPasswordResetToken.mockReturnValue('resetToken');
//             userModel.findOne.mockResolvedValue(mockUser);
//             sendForgotPasswordEmail.mockResolvedValue({ success: true });
//         });
...
//                 'http://example.com/user/reset-password/resetToken ',
//                 'john@example.com'
//             );
//             expect(res.status).toHaveBeenCalledWith(200);
//             expect(res.json).toHaveBeenCalledWith({
...
//         it('should return 500 if email sending fails', async () => {
//             sendForgotPasswordEmail.mockResolvedValue({ success: false, error: 'Email sending error' });
//             await forgotPassword(req, res);
//             expect(res.status).toHaveBeenCalledWith(500);
//             expect(res.json).toHaveBeenCalledWith({
//                 success: false,
...
//     describe('resetPassword', () => {
//         beforeEach(() => {
//             req.params = { token: 'validResetToken' };
...
//             mockUser.passwordResetExpiresAt = undefined;
//             userModel.findOne.mockResolvedValue(mockUser);
//         });
...
//             expect(userModel.findOne).toHaveBeenCalledWith({
//                 passwordResetToken: 'hashedToken',
//                 passwordResetExpiresAt: { $gt: expect.any(Number) }
//             });
...
//     describe('updatePassword', () => {
//         beforeEach(() => {
//             req.body = { passwordCurrent: 'CurrentPassword123', password: 'NewPassword123' };
//             req.user = { ...mockUser, correctPassword: jest.fn().mockResolvedValue(true) };
//         });

//         it('should update password successfully', async () => {
//             await updatePassword(req, res);
//             expect(req.user.correctPassword).toHaveBeenCalledWith('CurrentPassword123', 'hashedPassword');
//             expect(req.user.password).toBe('NewPassword123');
//             expect(req.user.save).toHaveBeenCalled();
//             expect(generateTokens).toHaveBeenCalled();
//             expect(res.status).toHaveBeenCalledWith(200);
//             expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
...
//             req.user.correctPassword.mockResolvedValue(false);
//             await updatePassword(req, res);
//             expect(req.user.correctPassword).toHaveBeenCalled();
//             expect(res.status).toHaveBeenCalledWith(401);
//             expect(res.json).toHaveBeenCalledWith({ message: 'your current password is wrong' });
//         });
...
//         it('should handle unexpected errors', async () => {
//             console.log = jest.fn();
//             req.user.save.mockRejectedValue(new Error('Database error'));
//             await updatePassword(req, res);
//             expect(console.log).toHaveBeenCalled();
//         });
//     });
...
//     describe('updateEmail', () => {
//         beforeEach(() => {
//             req.body = { newEmail: 'newemail@example.com', password: 'Password123' };
...
//             expect(mockUser.isConfirmed).toBe(false);
//             expect(mockUser.save).toHaveBeenCalled();
//             expect(sendEmailConfirmation).toHaveBeenCalledWith('mockUserId');
//             expect(generateTokens).toHaveBeenCalled();
//             expect(res.status).toHaveBeenCalledWith(200);
//             expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
//                 status: 'success',
...
//     describe('updateName', () => {
//         beforeEach(() => {
//             req.body = { firstName: 'NewFirstName', lastName: 'NewLastName' };
